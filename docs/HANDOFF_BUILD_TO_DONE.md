...
**E1 — Asset architecture (locked).** Migrate off the shared-placeholder system to **Astro assets**. Each entry's plate is a real file at `src/assets/plates/<collection>/<slug>.{webp,jpg}`, referenced through the content schema via Astro `image()` (the schema's `glyph` string was the placeholder seam for exactly this). **First read the current `src/content.config.ts`, `src/lib/plates.ts`, and `src/styles/base.css`** to confirm present wiring before changing anything — do not assume the state from this doc. Update the schema + `plates.ts`. **Keep the 9 seed plates as a clearly-labeled, visibly-provisional transitional fallback** for entries not yet sourced (no live regression) — they still render grayscale until Phase F, so they read as placeholders, not finished art. Do **not** retire the seed mapping in E1; that happens in E5.

**E2 — Source one image per entry**, working **in category batches** (all of one category, then the next) so each section's grid flips from seed to real plate as a unit rather than a patchwork. Covers symbols, figures, casebook, pillars, and timeline nodes. Source from `docs/05_IMAGE_SOURCES.md`: Wellcome Collection, Wikimedia Commons, Internet Archive, and the open-access museum APIs (Met, Cleveland, AIC, Smithsonian, Getty, Europeana). Choose the actual engraving/plate/portrait the entry discusses where one exists. **Verify each image is genuinely public-domain / CC0** — if license is unclear, skip it and list the entry for follow-up; never ship an unverified image. **Follow `docs/IMAGE_SOURCING.md` for all sourcing — two-pass manifest+fetch, compliant UA, Retry-After, host rotation, SVG-recreate geometric symbols.**
